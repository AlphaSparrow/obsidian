...
///
/// Algorithmic Basis (Dmitry Vyukov's Bounded MPMC Algorithm):
...
///   - Time Complexity: O(1) wait-free progression under low contention,
...
public:
    ...
    explicit LockFreeMPMCQueue(size_t capacity)
        : capacity_(next_power_of_two(capacity < 2 ? 2 : capacity)),
          mask_(capacity_ - 1),
          buffer_(static_cast<Cell*>(memory::cache_aligned_alloc(
              capacity_ * sizeof(Cell),
              alignof(Cell) > kCacheLineSize ? alignof(Cell) : kCacheLineSize))) {
        assert(buffer_ != nullptr && "Failed to allocate cache-aligned buffer");
    ...
    /// Batch dequeue up to max_items. Returns number of items dequeued.
    size_t dequeue_bulk(T* destination, size_t max_items) {
        size_t count = 0;
        while (count < max_items && dequeue(destination[count])) {
            ++count;
        }
        return count;
    }
    ...
    OBSIDIAN_NODISCARD bool empty() const noexcept {
        const size_t ep = enqueue_pos_.load(std::memory_order_relaxed);
        const size_t dp = dequeue_pos_.load(std::memory_order_relaxed);
        return ep <= dp;
    }

    OBSIDIAN_NODISCARD size_t approximate_size() const noexcept {
        const size_t ep = enqueue_pos_.load(std::memory_order_relaxed);
        const size_t dp = dequeue_pos_.load(std::memory_order_relaxed);
